...
export default function ShopPage() {
  ...
  return (
    <div className="min-h-screen bg-cream font-body">
      ...
      {/* Filter tabs — sticky */}
      <div className="sticky top-16 z-30 bg-white border-b border-cream-dark shadow-sm">
        <div className="max-w-5xl mx-auto px-6 py-4 flex gap-2 overflow-x-auto scrollbar-none">
          {productCategories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setActiveFilter(cat.id)}
              className={`flex-shrink-0 px-4 py-3 rounded-full text-sm font-medium transition-all duration-200 ${
                activeFilter === cat.id
